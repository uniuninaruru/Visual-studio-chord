...
1. 初回チュートリアルを開始する。不要ならスキップできます（あとから左上の**3本線 → 使い方**でいつでも読み返せます）
2. 左側の **Basic** を開く
...
学習と品質評価が必要で、正直自分の知識も全く足らないのでそこへ進む前に区切りをつけた形です。

正直なところ、909曲・約5Kwindowという規模に対してモデルが大きすぎることも
分かってきました。検証では6つの出力のうち4つが自明な多数派予測から動かず、
学習は5エポックで頭打ちになりました。この規模のデータで意味のある品質へ
...
- 最長の長さでは**落ちサビ・大サビ**まで含む完全なJ-POP形式（2周 → ブリッジ → 落ちサビ → 大サビ）。同じサビの進行を、静かに、そして最大で鳴らします
- **1区間4小節（1楽節）を下限**とし、下回るくらいなら区間数を減らします
- 最終セクションの**転調**（半音・全音・短3度上などのキー変化）
- メロディが和声と異なるモードを取る**複調**、ヨナ抜き/ニロ抜き**ペンタトニック旋律**
- 固定シードによる再現可能なコード・メロディ生成
- 旋律のリズムは**拍子が許す音価**（全〜16分、付点を含む8種）から選ばれ、楽句の終わりには着地する長さの音が確保されます
- 強拍・フレーズ端をコードトーンへ置き、経過音や刺繍音は前後関係から説明できる場合だけ残す旋律品質チェック
- 大跳躍後の反対方向への順次進行、フレーズ頂点、音域の弧を考慮した歌える旋律
- A/B/C候補をプレビューし、採用時だけ正式データへ反映
- **ライト／ダーク／端末に合わせる**の3択テーマ（メニュー →設定 →外観）。全色がトークン化されているので、コードレーンもピアノロールも両テーマで設計されています
- 三機能の色は**1型・2型色覚でも区別できる**よう選定（トニック／サブドミナントの色差は模擬で 9.3 → 25.8）
- **好みを生成に反映**（既定オフ）— 生成ボタンが複数案を引き、A/B評価で学習した好みに近いものを残します。勝った案のシードが曲に記録されるので、**そのシードだけで、学習なしに同じ曲を再現できます**
- 約1500件のコード進行検索から、結果を**選択中のセクションへそのまま適用**（小節は自分の和声リズムを保ち、各和音が何を綴るかだけが変わります）
- **なぜそう書いたかの説明**を、和音単位と曲全体の両方で表示（各記述が根拠となる理論分野を明示）
- 編集画面は「コードレーン → ピアノロール → 道具（直す／探す／置き換える／比べる／記録）」の3ブロック。**次の操作が何に効くか**を曲名の下に常時表示します

**曲づくりアシスト（Advanced画面から使えます）**

Advancedタブの「曲の流れ（Phase A）」「歌えるメロディ（Phase B）」「和音を豊かに（Phase C）」「ノリと多声部（Phase D）」から選べます。各項目には「何が変わるか」を日本語で表示します。

いずれも**個別に切り替えられる設定**で、切ればその機能が存在しなかったときの生成結果に戻ります。**出荷時の既定ではこれらの多くが有効です** — かつては一つずつ手で有効にしないと使われず、押しただけでは平三和音と2種類の強弱しか出ませんでした。
...
ボイシングは**指定するものではなく、曲に合わせて選ばれるもの**です。設定でシェイプを名指しすることはできません。和音が取りうる全シェイプを全音域に配置し、コストが最も低いものが勝ちます。

- **シェイプのカタログ** — close、drop2 / 3 / 2+4、spread、shell、rootless A / B、quartal、根音・5度の重複、両手ボイシング（広い形と、C3-G3 の上に E4-G4-C5 を置く普通の形）、compact
- **低音程限界** — 低い音域で狭い音程を重ねると和音ではなく濁りとして聞こえます。臨界帯域に基づく標準的な表を持ち、これは好みではなく**規則**として扱います（他のどの項よりも優先）
- **メロディを覆わない** — 耳は最高音を旋律として取るので、和音の最高音がメロディに達すると書いたはずの旋律が消えます
- **間隔** — 下ほど広く上ほど密に。両手ボイシングの「穴」は片手ずつ判定するので、間隔の規則が最も普通の形を却下しません
- **セクションごとの音域** — サビはAメロより高い位置で鳴ります。音量の弧（intro 0.72 → chorus 1.06）と同じ主張を、手の位置についても行います
- **アルペジオはペダルのように保持** — 保持しないアルペジオは単旋律で、上流のボイシングを全部捨てます
...
- **メロディMIDIの読み込み** — 単旋律のMIDIから調を推定し、合うコードを付けた曲にします。伴奏込みのファイルは最もメロディらしい1トラックを選びます。フォーマット2とSMPTEタイムは、推測すると全ノートがずれるので理由を述べて断ります（読み込みに失敗しても開いている曲は変わりません）
- プロジェクトは localStorage → セッション内メモリ、好み学習は IndexedDB → localStorage → メモリへ段階的にフォールバック

**UIと環境**

- 環境診断、初回チュートリアル、Basic / Advanced / Developer 設定
- **メニュー（左上の3本線）** — 使い方ガイド、更新履歴、ライセンス表記、音量設定（全体・コード・メロディ・ベース・リバーブ）。音量はこの端末に保存され、プロジェクトにも書き出したJSONにも入りません
...
ブラウザが音声の再生をブロックしている可能性があります。画面のどこかをクリックしてから Play を押し直してください。

音は出るが小さい・大きい場合は、左上の **3本線 → 設定** に全体とトラック別の音量があります。設定はこの端末に保存され、曲そのものには影響しません。

改善しない場合は右上の **Diagnostics** で音声まわりの状態を確認できます。
