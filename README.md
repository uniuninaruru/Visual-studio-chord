...
- 対旋律・カノン・ポリリズム低音を重ね、声部ごとに再生／ミュート／MIDI書き出し
...
画面中央の「この曲を診断」を押すと、現在の緊張カーブ、フレーズ設定、ボイスリーディング、グルーヴ、多声部の状態を調べ、実装済み機能だけで修正版を作ります。
...
- Tone.js再生（声部別音色、ミュート対応）
- ピアノロール（声部別の色、主旋律は従来どおり編集可能）
- Standard MIDI File（Chords、Melody、各追加声部を別トラック化）
...
- JSONプロジェクト入出力、コード＋主旋律＋追加声部のマルチトラックMIDI出力
